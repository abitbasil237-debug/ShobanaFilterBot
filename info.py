...
BOT_TOKEN = environ.get('BOT_TOKEN', '8706884060:AAGSWihoihaAcGre8wDesuI02OM1A245ZDE'')
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '8015617397').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003836488317').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '8015617397').split()]
...
FILE_CHANNELS = [int(ch) for ch in environ.get('FILE_CHANNELS', '-1003812011442').split()]
